...
def go_to_corner(PointA,radius,squares,distance):
    ...
    PointB[1] = decrementlon(90,distance,PointA[1],distanceindegree(PointA[1],PointA[0]))

  #  lat1 = incrementlat(-180,.125/4,lat1,distlat(lat1)) 
    make_square(PointB[0],PointB[1],squares,distance)
def distlat(lat1):
...
def make_square(lat1,lon1,count,btwnmarks):
    originallong = lon1
    y = 1
    x = 0
    global square_count
    ...
    while(x <= numofiterations(square_count)):
        lat1 = incrementlat(-180,float(btwnmarks/1.5),lat1,distlat(lat1))  
        print str(lat1) + "," + str(lon1)
        x += 1
    while (x > 0):
        lon1 = incrementlon(-225,float(btwnmarks),lon1,distanceindegree(lon1,lat1)) #Go directly .05 miles to the right and query
        print str(lat1) + "," + str(lon1)
        x -= 1
    while (x <= numofiterations(square_count)):
        lat1 = decrementlat(180,float(btwnmarks/1.5),lat1,distlat(lat1)) 
        print str(lat1) + "," + str(lon1)
        x += 1
    x -= 1    
    lastlat = lat1
    ...
    while (x > 0):
        lon1 = decrementlon(45,float(btwnmarks),lon1,distanceindegree(lon1,lat1))        
        print str(lat1) + "," + str(lon1)
        x -= 1
    if (count > 0):
        go_to_corner((firstlat,firstlon),1,count-1,btwnmarks)
...
for line in inputfile:
  ...
  iterate_counter=0
  PointA = go_to_corner(PointA,1,5,.5)
