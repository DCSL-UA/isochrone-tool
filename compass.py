...
from getlat import latnumbers, longnumbers, makekeys, lastrun, checknumbers, cleanprint, cleanlist, converter, kmlmaker, polyarea
import googlemaps
import json
...
def gmaps_traveltimeordist(directions11, last, timeordist):
  ...
  if len(directions11) > 0 and timeordist == 0:
    print("DIST : " + str(directions11[0]["legs"][0]["duration"]["value"]))
...
def my_algorithm(d, distance, Initial_increment, goaltime, InitialPoint, mode, modes_to_run, output, KEYS, degreeincrements, timeordist):
...
#  print "My Algorithm"
 ...
 #   print "LIST IS " + str(printlist
    ...
    print("Pointa distance = " + str(get_distance(float(InitialPoint[0]), float(InitialPoint[1]), \
      float(pointa.split(",")[0]), float(pointa.split(",")[1]))))
...
for line in inputfile:
  ...
  go_to_corner(PointA, 1, 1, .5, currentname, 0, 15)   #This gets 3 pairs that are 10,20, and 30 miles from origin on bearing
  iterate_counter = 0
  thelist = my_algorithm(currentname, .5, 1, 1400, PointA, "driving", ["driving"], output, KEYS, 15, 1)   #Last parameter is 1 = time, 0 = distance
  print(thelist)
  mypoints = converter(thelist)
  print(mypoints)
  kmlmaker(mypoints, "final2", thelist)
  polyarea("final2.kml")
  cleanprint(thelist, 1400)
#  for pnt in mypoints:
